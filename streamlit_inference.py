...
# # ⚠️ Phải là dòng đầu tiên liên quan đến Streamlit
# del st.session_state  # Khởi tạo sạch sẽ nếu có cache
# st.set_page_config(page_title="Ultralytics Streamlit App", layout="wide")

# # Class xử lý frame từ webcam (qua trình duyệt)
# class VideoProcessor(VideoTransformerBase):
#     def __init__(self, model: YOLO, conf: float, iou: float, classes: list, enable_trk: bool):
#         self.model = model
#         self.conf = conf
#         self.iou = iou
...
#         img = frame.to_ndarray(format="bgr24")
#         if self.enable_trk:
#             results = self.model.track(img, conf=self.conf, iou=self.iou, classes=self.classes, persist=True)
#         else:
...
#     def run(self):
#         st.markdown("""
#         <h1 style='text-align:center; color:#FF64DA;'>Ultralytics YOLO Streamlit App</h1>
#         <h4 style='text-align:center; color:#042AFF;'>Realtime detection via webcam using YOLO 🚀</h4>
#         <hr>
#         """, unsafe_allow_html=True)

#         self.sidebar()
#         class_names = self.load_model()
...
#         st.subheader("📷 Realtime Webcam Feed")
#         webrtc_streamer(
#             key="streamlit-yolo",
#             video_transformer_factory=lambda: VideoProcessor(
#                 model=self.model,
#                 conf=self.conf,
#                 iou=self.iou,
